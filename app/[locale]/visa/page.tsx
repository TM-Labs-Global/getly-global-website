...
import { FileCheck, ArrowUpRight, CheckCircle2 } from "lucide-react";
import { APP_STORE_URL } from "@/shared/utils/appLinks";
import { JsonLd } from "@/shared/components/JsonLd";
import ProductHero from "@/shared/components/ProductHero";
import VisaHowItWorks from "@/features/visa/components/VisaHowItWorks";
...
export default async function VisaPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "visa" });
  const tPillar = await getTranslations({ locale, namespace: "home.pillars.visa" });
  ...
  return (
    <>
      ...
      <ProductHero
        ...
        imageAlt="Traveler checking digital visa status on smartphone"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center bg-white rounded-3xl p-8 md:p-12 border border-[var(--line)] shadow-soft">
            ...
            <div className="relative w-full h-[420px] rounded-2xl overflow-hidden bg-[#07153d] border border-white/10 flex items-center justify-center p-8 shadow-inner">
              <div className="relative w-64 h-64">
                <Image
                  src="/3d/travel/passport.png"
                  alt="Getly Visa Passport"
                  fill
